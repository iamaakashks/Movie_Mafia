                ...
                <div className="bg-[#141619] border-[1px] border-t-0 border-[#2c2e3a] w-2/5 max-h-96 absolute top-[82%] overflow-auto">
                    {
                        searchList.map((s, index) => {
                            return <Link key={index} to="/" className="text-[#b3b4bd] flex gap-4 items-center hover:bg-[#2c2e3a] line-clamp-4 duration-200 p-4">
                            <img className="w-16 h-16 rounded-md object-cover" src={s.backdrop_path || s.profile_path ? `https://image.tmdb.org/t/p/original/${s.backdrop_path || s.profile_path}` : `../../../public/noImage.jpg`} alt="" />
                            <h4 className="font-bold text-lg leading-tight">{s.title || s.name}</h4>
                        </Link>
                        })
                    }
                </div>
